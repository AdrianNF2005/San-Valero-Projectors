...
let imageDisplayDuration = 20000; // Duración de cada imagen (3 segundos)
...
let TiempoUnderProyector = 10000;
...
let TiempoPirDebeEstarEncendido = 16000;
...
function preload() {
  ...
  VIDEOInicioProyector = createVideo("https://raw.githubusercontent.com/AdrianNF2005/San-Valero-Projectors/main/ASSETS%20PROYECTORES/PantallaInicio/VideoInicio.mp4", videoLoaded); // Carga el video
...
function videoLoaded() {
  VIDEOInicioProyector.play(); // Inicia el video automáticamente
}
...
function draw() {
  ...
  if (!started) return; // No hace nada hasta que started sea true
  ...
  if (VIDEOInicioProyector.time() <= 59) {
    ...
    if (ActivarVideoLoops == true) {
      
      video.loop();
      VIDEOCreditos.loop();
     
      ActivarVideoLoops = false;
  ...
  if (CalibracionPantalla == 1) {
    ...
    tint(255, map(sin(frameCount), -1, 1, 0, 255));
  ...
  if (Etapa <= -1) {
    ...
    if (currentCategoryIndex == 0) {
      ...
      fill(171, 235, 198);
      textFont(Fuente3);
      strokeWeight(VolumenAltavoz/100);
      stroke(171, 235, 198);
      text(
        "ACTUALIDAD",
        width * 0.82,
...
function GameMenu() {
  if (Etapa == 0) {
    ...
    if (URLOpened == false) {
      ...
      ref = database.ref("Proyectores/PuntoMinimo");

      ref.set(round(MinYmouse));
      
      ref = database.ref("Proyectores/PuntoMaximo");
      ...
      if (mouseY <= height*0.3 && OpacidadAllIcons >= 1000) {
        ...
        if (posicionIMG == 2 && posicionIMGini == 2) {
          ...
          abrirIframe(VolumenAltavoz/100);
        } else if (posicionIMG == 4 && posicionIMGini == 4) {
          abrirIframe(2);
        } else if (posicionIMG == 5 && posicionIMGini == 5) {
          abrirIframe(3);
